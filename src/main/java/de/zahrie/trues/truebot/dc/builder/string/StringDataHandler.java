...
import de.zahrie.trues.truebot.util.database.CellEntry;
...
public class StringDataHandler {
  ...
  private String create(Object[] entry, boolean enumerate) {
    ...
    for (ColumnData wrapper : lengths) {
      ...
      String cell = String.format((column.leftAligned() ? "" : "-") + "%" + wrapper.getSubLengths().get(0) + "s", baseEntry.round(column));
      if (wrapper.isTwoInOne()) {
        final var additional = new CellEntry(entry[index + 1]);
        column = query.columns()[index + 1];
        final String cell2 = String.format((column.leftAligned() ? "" : "-") + "%" + wrapper.getSubLengths().get(1) + "s", additional.round(column));
        cell += " (" + cell2 + ")";
      }
      keys.add(cell);
    }
