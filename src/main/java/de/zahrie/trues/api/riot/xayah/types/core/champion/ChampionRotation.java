...
public class ChampionRotation extends GhostObject<de.zahrie.trues.api.riot.xayah.types.data.champion.ChampionRotation> {
  ...
  private final java.util.function.Supplier<SearchableList<RiotChampion>> freeChampions = Suppliers.memoize(() -> {
    load(CHAMPION_ROTATION_LOAD_GROUP);
    return coreData.getFreeChampionIds() == null ? null : SearchableLists
        .unmodifiableFrom(Champions.withIds(coreData.getFreeChampionIds()).withPlatform(Platform.withTag(coreData.getPlatform())).get());
  })::get;

  private final Supplier<SearchableList<RiotChampion>> freeChampionsForNewPlayers = Suppliers.memoize(() -> {
    load(CHAMPION_ROTATION_LOAD_GROUP);
    return coreData.getFreeChampionIdsForNewPlayers() == null ? null : SearchableLists
  ...
  public SearchableList<RiotChampion> getFreeChampions() {
    return freeChampions.get();
  }

  public SearchableList<RiotChampion> getFreeChampionsForNewPlayers() {
