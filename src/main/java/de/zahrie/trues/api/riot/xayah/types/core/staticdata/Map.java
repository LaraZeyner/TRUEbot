...
public class Map extends GhostObject<de.zahrie.trues.api.riot.xayah.types.data.staticdata.Map> {
  ...
  private final Supplier<SearchableList<Item>> unpurchasableItems = Suppliers.memoize(() -> {
    load(MAP_LOAD_GROUP);
    return coreData.getUnpurchasableItems() == null ? null : SearchableLists.unmodifiableFrom(Items.withIds(coreData.getUnpurchasableItems()).withPlatform(Platform.withTag(coreData.getPlatform())).withVersion(coreData.getVersion()).withLocale(coreData.getLocale()).get());
  })::get;
  ...
  public de.zahrie.trues.api.riot.xayah.types.common.Map getMap() {
    ...
    return de.zahrie.trues.api.riot.xayah.types.common.Map.withId(coreData.getId());
  }
