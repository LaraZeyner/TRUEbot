...
import de.zahrie.trues.api.riot.xayah.types.common.MasteryTree;
...
public class Mastery extends GhostObject<de.zahrie.trues.api.riot.xayah.types.data.staticdata.Mastery> {
  ...
  private final Supplier<List<String>> descriptions = Suppliers.memoize(() -> {
    ...
    return coreData.getDescriptions() == null ? null : Collections.unmodifiableList(coreData.getDescriptions());
  })::get;
  ...
  private final Supplier<Mastery> prerequisite = Suppliers.memoize(() -> {
    load(MASTERY_LOAD_GROUP);
    return coreData.getPrerequisite() == 0 ? null : Mastery.withId(coreData.getPrerequisite()).withPlatform(Platform.withTag(coreData.getPlatform())).withVersion(coreData.getVersion()).withLocale(coreData.getLocale()).get();
  })::get;

  private final Supplier<List<String>> sanitizedDescriptions = Suppliers.memoize(() -> {
    load(MASTERY_LOAD_GROUP);
    return coreData.getSanitizedDescriptions() == null ? null : Collections.unmodifiableList(coreData.getSanitizedDescriptions());
  })::get;
  ...
  protected List<String> getLoadGroups() {
    return List.of(MASTERY_LOAD_GROUP);
  }
  ...
  protected void loadCoreData(final String group) {
    ...
    if (group.equals(MASTERY_LOAD_GROUP)) {
      ...
      final de.zahrie.trues.api.riot.xayah.types.data.staticdata.Mastery data =
          Orianna.getSettings().getPipeline().get(de.zahrie.trues.api.riot.xayah.types.data.staticdata.Mastery.class, builder.build());
      if (data != null) {
        coreData = data;
      }
    }
