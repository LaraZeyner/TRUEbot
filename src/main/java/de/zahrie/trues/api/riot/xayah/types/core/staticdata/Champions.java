...
public class Champions extends GhostObject.ListProxy<RiotChampion, Champion, de.zahrie.trues.api.riot.xayah.types.data.staticdata.Champions> {
  ...
  public static final class SubsetBuilder {
    ...
    public SearchableList<RiotChampion> get() {
      if (platform == null) {
        platform = Orianna.getSettings().getDefaultPlatform();
        if (platform == null) {
          throw new IllegalStateException(
      ...
      final CloseableIterator<RiotChampion> result =
          Orianna.getSettings().getPipeline().getMany(RiotChampion.class, builder.build(), streaming);
  ...
  protected void loadCoreData(final String group) {
    ...
    if (group.equals(LIST_PROXY_LOAD_GROUP)) {
      ...
      loadListProxyData(data1 -> {
        final RiotChampion riotChampion = new RiotChampion(data1);
        riotChampion.markAsGhostLoaded(RiotChampion.CHAMPION_LOAD_GROUP);
        return riotChampion;
      });
    }
  }
