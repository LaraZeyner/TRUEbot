...
public class LineupFactory {
  public static Lineup determineLineup(Participator participator, Player player) {
    return QueryBuilder.hql(Lineup.class, "FROM Lineup WHERE participator = " + participator + " AND player = " + player).single();
  }

  public static Lineup determineLineup(Participator participator, Lane lane) {
    return QueryBuilder.hql(Lineup.class, "FROM Lineup WHERE participator = " + participator + " AND lane = " + lane).single();
  }
