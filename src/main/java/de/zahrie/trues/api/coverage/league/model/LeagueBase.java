...
@Getter
@Setter
public abstract class LeagueBase implements ALeague, Id {
  ...
  public List<LeagueMatch> getMatches() {
    return new Query<LeagueMatch>().where("coverage_group", this).entityList();
  }

  public List<LeagueTeam> getLeagueTeams() {
    return new Query<LeagueTeam>().where("league", this).entityList();
  }

  public List<LeagueTeam> getSignups() {
    return new Query<LeagueTeam>().where("league", this).entityList().stream().toList();
  }
